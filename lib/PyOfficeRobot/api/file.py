...
def send_file_new(who: str, file: str):

    wx.ChatWith(who)  # 打开聊天窗口
    # wx.SendFiles(file)
    wx.test_SendFiles_new(filepath=file, who=who)  # 添加who参数：雷杰
...
def get_wechat_info():
    ...
    rect = nums.GetParentControl().BoundingRectangle  # 获取父级控件的边框范围

    # 获取第一个聊天成员
    first_member = nums.GetFirstChildControl()
    if first_member and isinstance(first_member, uia.ListItemControl):
        user_info = {}
        user_info['微信资料'] = first_member.Name
        first_member.Click(waitTime=0.01)

        user_info_pane = uia.PaneControl(Name="微信")
        for control, depth in uia.WalkControl(user_info_pane):
            if control.ControlTypeName == "TextControl" and "：" in control.Name:
                key = control.Name.rstrip("：")
                value = control.GetNextSiblingControl().Name
                user_info[key] = value
...
def get_group_list():
    ...
    if more_button.Exists(0, 0):
        more_button.Click(-80, -30, simulateMove=False)
    ...
    numbers.Click(waitTime=0.5)  # 点击一下列表控件,不然下面获取的控件会有问题
    sum_info = []
